...
const ImportExcelDialog: React.FC<ImportExcelDialogProps> = ({
  ...
  const downloadTemplate = () => {
    const template = [
      {
        'Loại Nhập': 'Nhập hàng',
        'Ngày Nhập': '2025-07-27',
        'Khách Hàng': 'Khách hàng A',
        'Mã Hóa Đơn': 'HD001',
        'Số Lượng Sản Phẩm': 10,
        'Số Lượng Xuất': 8,
        'Tài Xế': 'Nguyễn Văn A',
        'Nội Dung Nhập': 'Nhập hàng từ nhà cung cấp',
        'Ghi Chú': 'Ghi chú mẫu',
      },
      {
        'Loại Nhập': 'Nhập trả',
    ...
    const colWidths = [
      ...
      { wch: 18 }, // Số Lượng Xuất
      { wch: 15 }, // Tài Xế
      { wch: 30 }, // Nội Dung Nhập
      { wch: 25 }, // Ghi Chú
    ];
    ws['!cols'] = colWidths;
  ...
  const validateData = (data: any[]): { valid: boolean; errors: string[] } => {
    ...
    data.forEach((row, index) => {
      ...
      if (!row.loai_nhap) {
        errors.push(`Dòng ${rowNum}: Thiếu Loại Nhập`);
      }
      if (!row.ngay_nhap) {
        errors.push(`Dòng ${rowNum}: Thiếu Ngày Nhập`);
      }
      ...
      if (isNaN(row.sl_xuat) || row.sl_xuat < 0) {
        errors.push(`Dòng ${rowNum}: Số Lượng Xuất không hợp lệ`);
      }
    });
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const mappedData: ImportData[] = jsonData.slice(1).map((row: any) => ({
          loai_nhap: row[0] || '',
          ngay_nhap: row[1] ? new Date(row[1]).toISOString().split('T')[0] : '',
          ten_khach_hang: row[2] || '',
          ma_hoa_don: row[3] || '',
          sl_san_pham: parseInt(row[4]) || 0,
          sl_xuat: parseInt(row[5]) || 0,
          tai_xe: row[6] || '',
          noi_dung_nhap: row[7] || '',
          ghi_chu: row[8] || '',
        }));
    ...
    <Dialog open={open} onClose={handleClose} maxWidth="lg" fullWidth>
      <DialogTitle>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          <UploadIcon />
          <Typography variant="h6">Import Excel - Nhập Kho</Typography>
        </Box>
      </DialogTitle>
          ...
          {isValid && (
            <Alert severity="success" sx={{ mb: 2 }}>
              <Typography variant="subtitle2">
                ✅ Dữ liệu hợp lệ! Có {importedData.length} phiếu nhập kho để import.
              </Typography>
            </Alert>
          )}
        </Box>
